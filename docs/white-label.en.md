...
A guide for whoever installs Elev CRM **for other companies** — agency, consultancy, reseller — and charges for it.
...
- **Two technical names do not change**: the `X-Elev CRM-Signature` header of outbound webhooks and the session cookie. The first is a contract with third-party systems that already check that name; renaming it would break a client's integration **silently** — the receiver raises no error, it merely stops recognizing.
