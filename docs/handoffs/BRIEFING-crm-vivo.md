...
- **Worktree:** `/Users/rafaelmelgaco/ElevCRM-crm-vivo`
...
Existem outros worktrees vivos (`.claude/worktrees/*`, `Elev CRM-qa`, `Elev CRM-main-preview`,
`Elev CRM-vendaval*`). **Nenhum deles é seu.** Trabalhe só no worktree acima.

---

## 1. A missão

O Elev CRM existe para que **todo lead que entra vire uma venda feita ou um problema
...
`ELEVCRM_GOV_INVARIANTS_EDIT=1` com o `+N −0` **medido** no corpo do commit. Se o `−0` não
