# ADR-0001 — Packaging e distribuição do Elev CRM
...
O Elev CRM é distribuído como self-host: a monetização é a venda da VPS com o sistema
...
|---|---|---|
| `elevcrm` | "o que a pessoa instala?" | existe desde 2026-07-02 |
| `elevcrm-worker` | "o que roda 24/7 fora do request?" | **criado aqui** |
| `elevcrm-scheduler` | "o que dispara os crons?" | **criado aqui** |
...
### D3 — `elevcrm-worker` é publicado, não fundido na imagem do app
...
| "o compose aponta para `ghcr.io/elevcrm/elevcrm`" | aponta para `ghcr.io/melgarafael/elevcrm`. A org `elevcrm` não existe (404). A string aparecia num `git clone` de `docs/deploy-selfhost/README.md` — link quebrado, corrigido aqui |
