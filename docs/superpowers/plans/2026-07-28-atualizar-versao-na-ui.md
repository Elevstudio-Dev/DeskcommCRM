...
**Goal:** O dono de uma instalação self-host atualiza o Elev CRM clicando num botão na própria tela, sem abrir terminal nem SSH.
...
- **Worktree:** `~/ElevCRM-update`, branch `feat/atualizar-pela-ui` (já criada a partir de `origin/main`). Não trabalhe no clone principal — ele está sujo com trabalho de outra sessão.
...
Primeira versão marcada do Elev CRM.
`;
...
const COMANDO_MANUAL = "cd Elev CRM && bash hostgator-setup-kit/update.sh";
...
set +e
ELEVCRM_AGENT_REPORT=1 \
ELEVCRM_AGENT_PREV_IMAGE="$PREV_IMAGE" \
ELEVCRM_AGENT_REPORT_CMD="$(declare -f report); report" \
...
export APP_IMAGE="ghcr.io/melgarafael/elevcrm:${TARGET_TAG#v}"
...
```bash
[ -n "${ELEVCRM_AGENT_REPORT:-}" ] && eval "${ELEVCRM_AGENT_REPORT_CMD}" backup   # após o backup
[ -n "${ELEVCRM_AGENT_REPORT:-}" ] && eval "${ELEVCRM_AGENT_REPORT_CMD}" codigo   # após o checkout
[ -n "${ELEVCRM_AGENT_REPORT:-}" ] && eval "${ELEVCRM_AGENT_REPORT_CMD}" banco    # após o baseline
```
