# Atualizar o Elev CRM pela própria tela
...
Quem hospeda o Elev CRM numa VPS atualiza hoje assim: abre um terminal, conecta por SSH
...
O app roda dentro de um container (`ghcr.io/melgarafael/elevcrm`) sem nenhum volume do host.
...
- Sobe `APP_IMAGE=ghcr.io/melgarafael/elevcrm:<versão>` em vez de `latest`.
