...
- **(A) RECOMENDADA — projeto Supabase NOVO dedicado** ("elevcrm-fusion-vps",
...
POST https://api.supabase.com/v1/projects
  {"name":"elevcrm-fusion-vps","organization_id":"pzaxvazodcffkfkpyqtl",
...
# usuário via Auth Admin API (email elevcrm-vps-admin@..., senha gerada) +
...
ssh ... 'mkdir -p /opt/elevcrm-fusion && git clone -b vendaval-fusion /opt/fusion.bundle /opt/elevcrm-fusion/app'
...
scp -P 22022 -i ~/.ssh/... /tmp/fusion-vps.env root@...:/opt/elevcrm-fusion/app/.env
ssh ... 'chmod 600 /opt/elevcrm-fusion/app/.env && rm -f /opt/fusion.bundle'
...
#     AGENT_DISPATCH_CONSUMER=engine · WAHA key + sha512 · APP_IMAGE=elevcrm-app:vps
...
# rede/volumes: os default do project-name elevcrm-fusion (nada da easypanel)

# B4. build local na VPS (~10-20 min; 4 cores é ok)
ssh ... 'cd /opt/elevcrm-fusion/app && docker compose -p elevcrm-fusion \
...
ssh ... 'docker compose -p elevcrm-fusion ... ps'  # tudo healthy; ss -tlnp → só as novas portas
```
...
no firewall; rollback = `docker compose -p elevcrm-fusion down` (só a nossa stack).
