# Como subir o seu Elev CRM na HostGator (passo a passo, sem enrolação)
...
```
Seu domínio  →  Servidor VPS (HostGator)  →  Elev CRM rodando
                        │
                        ├─ o CRM (site + painel)
...
O Elev CRM roda num **VPS com Docker**. A opção mais fácil é um VPS que **já vem
...
2. Escreva pra ele exatamente isto:
   ...
   > *"Clone https://github.com/melgarafael/DeskcommCRM e me instale o Elev CRM
...
O Elev CRM foi desenhado pra rodar redondo na infraestrutura da HostGator. Além do
