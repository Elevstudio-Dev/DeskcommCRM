...
A `WAHA_API_KEY` do servidor é o **hash SHA512 hex (lowercase) do plaintext**. O backend Elev CRM guarda **só** o plaintext em Vercel Encrypted Env Var; nunca a hash duplicada. Geração:
...
WAHA_DASHBOARD_USERNAME=admin_elevcrm
WAHA_DASHBOARD_PASSWORD=<senha forte gerada>
WAHA_S3_REGION=auto
WAHA_S3_BUCKET=elevcrm-waha-media
...
WAHA_BASE_URL=https://waha.elevcrm.internal
WAHA_WEBHOOK_PUBLIC_BASE_URL=https://api.elevcrm.com
...
server {
  ...
  server_name waha.elevcrm.internal;

  ssl_certificate     /etc/letsencrypt/live/waha.elevcrm.internal/fullchain.pem;
  ssl_certificate_key /etc/letsencrypt/live/waha.elevcrm.internal/privkey.pem;
...
1. Verificar logs WAHA (`docker logs elevcrm-waha --tail 500`).
...
- Domínio gerado pela Railway (`waha-elevcrm.up.railway.app`); usado em `WAHA_BASE_URL` (Vercel).
- Custo: $5-10/mês.
...
- Instalação:
  ...
  certbot --nginx -d waha.elevcrm.internal
  cd /opt/elevcrm-waha && docker compose up -d
  ```
