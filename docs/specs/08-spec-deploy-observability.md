...
|---|---|---|---|---|---|
| Production | `main` | `app.elevcrm.com.br` + `admin.elevcrm.com.br` | projeto Pro `elevcrm-prod` | `waha.elevcrm.com.br` (Hostgator) | `production` |
| Staging | `staging` | `staging.elevcrm.com.br` | projeto Free `elevcrm-staging` | `waha-staging.elevcrm.com.br` (Hostgator mesmo VPS, container separado) | `staging` |
| Preview | qualquer PR | `*.vercel.app` | projeto Free `elevcrm-preview` (compartilhado) | mock/staging | `preview` |
...
- **Project:** `elevcrm-app` (Next.js) + `elevcrm-mcp` (MCP server fase 2).
...
```bash
git clone git@github.com:elevcrm/elevcrm.git
cd elevcrm
...
Migrations ficam em `supabase/migrations/YYYYMMDDHHMMSS_*.sql`. Seed em `supabase/seed.sql` cria 1 organization, 1 user `dev@elevcrm.local`, 1 pipeline default.
...
services:
  waha:
    ...
    container_name: elevcrm-waha-dev
...
ngrok http 3000 --domain=elevcrm-dev.ngrok.dev   # plano pago dá domínio fixo
```
...
```toml
[auth]
site_url = "https://app.elevcrm.com.br"
additional_redirect_urls = [
  "https://app.elevcrm.com.br/auth/callback",
  "https://admin.elevcrm.com.br/auth/callback",
]
...
- **Preview:** vars setadas em scope `Preview` apontando pra projeto Supabase `elevcrm-preview` compartilhado, WAHA mock ou staging, AI Gateway com `MOCK_AI=true` por default.
...
`/srv/elevcrm/docker-compose.yml`:
```yaml
services:
  waha:
    image: devlikeapro/waha-plus:latest
    container_name: elevcrm-waha
    restart: always
    networks:
      - waha-net
    expose:
    ...
    environment:
      ...
      WHATSAPP_DEFAULT_ENGINE: NOWEB
      WHATSAPP_HOOK_URL: https://app.elevcrm.com.br/api/webhooks/waha
  ...
  certbot:
    ...
    container_name: elevcrm-certbot
    volumes:
...
Backup script `/srv/elevcrm/scripts/backup-sessions.sh`:
```bash
#!/usr/bin/env bash
set -euo pipefail
...
docker compose -f /srv/elevcrm/docker-compose.yml stop waha
tar czf "$BACKUP_DIR/sessions-$TS.tar.gz" -C /srv/waha sessions
docker compose -f /srv/elevcrm/docker-compose.yml start waha
# Upload pro Wasabi/R2
aws s3 cp "$BACKUP_DIR/sessions-$TS.tar.gz" \
  "s3://elevcrm-backups/waha-sessions/sessions-$TS.tar.gz" \
...
server {
    listen 80;
    server_name waha.elevcrm.com.br;
...
server {
    ...
    ssl_certificate     /etc/letsencrypt/live/waha.elevcrm.com.br/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/waha.elevcrm.com.br/privkey.pem;
...
docker run --rm -v /etc/letsencrypt:/etc/letsencrypt -v /var/www/certbot:/var/www/certbot \
  ...
  -d waha.elevcrm.com.br --email ops@elevcrm.com.br --agree-tos --no-eff-email
```
...
- 1 deploy por commit. URL `elevcrm-app-git-{branch}-{team}.vercel.app`.
...
```json
{
  ...
  "service":"elevcrm-app","env":"production","commit":"a1b2c3d",
...
|---|---|---|---|---|---|
| A1 | Sentry novo issue (unhandled exception em prod) | 1ª ocorrência | Slack #elevcrm-alerts | warn | dev on-call |
| A2 | Sentry issue volume spike | >50 events/h | Slack + PagerDuty | crit | dev on-call |
| A3 | WAHA session FAILED | qualquer | Slack #elevcrm-ops + email tenant | crit | DevOps |
| A4 | WAHA session STARTING >5min | sustentado | Slack #elevcrm-ops | warn | DevOps |
| A5 | LGPD `data_request` SLA D+5 | cron diário | Slack #elevcrm-lgpd + email DPO | warn | LGPD owner |
| A6 | LGPD `data_request` SLA D+6 | cron diário | PagerDuty | crit | LGPD owner |
| A7 | Rate limit Nuvemshop estourado | >5 429s/min | Slack #elevcrm-ops | warn | dev on-call |
...
> Convenção: cada runbook tem **Sintoma → Diagnóstico → Ação → Verificação → Pós-mortem**. Salvos também em `/srv/elevcrm/runbooks/` no VPS pra acesso offline.
...
**Diagnóstico.**
1. Confirmar via WAHA dashboard interno (port-forward ssh) ou `docker exec elevcrm-waha curl localhost:3000/api/sessions`.
...
2. **Comunicação:** email ao admin do tenant + Slack #elevcrm-ops com root-cause hypothesis.
...
**Diagnóstico.**
```bash
ssh ops@waha.elevcrm.com.br
docker logs elevcrm-waha --tail 200 | grep -i "session-{id}"
docker exec elevcrm-waha ls /app/.sessions/{sessionId}
```
...
- **WAHA banimento generalizado da conta Elev CRM (modelo BPO):** plano de migração pra API oficial Meta documentado no PRD-03 (Fase 2.5).
