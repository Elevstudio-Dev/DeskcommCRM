...
|---|---|---|
| App embedded vs External | **External** | Mais flexibilidade de UI custom (admin Elev CRM tem UX própria); evita iframe sandbox; consent renderizado no domínio Nuvemshop é suficiente |
...
export class NuvemshopAdapter implements EcommercePlatformAdapter {
  ...
  private async request<T>(ctx: AdapterContext, path: string, init: RequestInit & { idempotencyKey?: string } = {}): Promise<{ data: T; rateLimit: FetchPage<T>['rateLimit'] }> {
    ...
    const headers: Record<string, string> = {
      ...
      'User-Agent': 'ElevCRM/1.0 (rafael@maudibrasil.com.br)',
...
- UI custom no admin Elev CRM (sem iframe Nuvemshop sandbox)
- Permite callback em domínio próprio com cookie de sessão Elev CRM
...
## 8. Mapping Nuvemshop → Elev CRM

### 8.1 `customer.email/phone/identification` → `contacts`

| Campo Nuvemshop | Campo Elev CRM | Notas |
|---|---|---|
