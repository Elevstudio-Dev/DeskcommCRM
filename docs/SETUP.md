...
- Fica na **raiz do projeto**: `/seu-caminho/ElevCRM/.env.local`.
...
2. No dashboard, clique **New project**.
   - **Name:** `elevcrm-dev` (ou o que quiser).
...
3. Clique **Create API Key** → nome `elevcrm-dev` → copie a chave.
...
3. **Settings → API Keys → Create Key** → nome `elevcrm-dev` → copie.
...
3. **API Keys → Create new secret key** → nome `elevcrm-dev-embeddings` → copie.
...
- Confira logs do container: `docker logs elevcrm-waha`.
