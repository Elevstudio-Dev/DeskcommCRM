...
2. Domínio com DNS gerenciado em Cloudflare (ou outro provider) — ex.: `waha.elevcrm.com.br`.
3. Conta Backblaze B2 com bucket `elevcrm-waha-backup` (R$0,06/GB/mês ≈ $0.005/GB).
...
# usuário não-root
adduser elevcrm
usermod -aG sudo elevcrm
mkdir -p /home/elevcrm/.ssh
cp ~/.ssh/authorized_keys /home/elevcrm/.ssh/
chown -R elevcrm:elevcrm /home/elevcrm/.ssh
chmod 700 /home/elevcrm/.ssh && chmod 600 /home/elevcrm/.ssh/authorized_keys
...
A partir daqui: `ssh elevcrm@<IP>` + `sudo` para tudo.
...
sudo usermod -aG docker elevcrm
...
```bash
sudo tee /etc/fail2ban/jail.d/elevcrm.conf > /dev/null <<'EOF'
[sshd]
enabled = true
maxretry = 3
bantime = 1h
...
- `waha.elevcrm.com.br` → A record → IP do VPS Hostgator
- Proxy = **DNS only** (cinza). Cloudflare Proxy (laranja) interfere em SSE/WebSocket que o WAHA usa.

```bash
sudo certbot --nginx -d waha.elevcrm.com.br \
  --non-interactive --agree-tos -m ops@elevcrm.com.br
```
...
```bash
sudo mkdir -p /opt/elevcrm-waha
sudo chown elevcrm:elevcrm /opt/elevcrm-waha
cd /opt/elevcrm-waha
```

Copiar (via `scp` ou `git clone`) o `docker-compose.yml` do repo (raiz do Elev CRM). Ajustes obrigatórios pra prod:
...
`/opt/elevcrm-waha/.env` (chmod 600):

```bash
WAHA_API_KEY=<plaintext gerado novo, 64 chars hex>
WAHA_WEBHOOK_BASE_URL=https://app.elevcrm.com.br
WAHA_HMAC_SECRET=<32 bytes random distinto da api key>
```

```bash
chmod 600 /opt/elevcrm-waha/.env
```
...
echo "0 4 * * * elevcrm /usr/local/bin/refresh-vercel-cidrs.sh" | sudo tee /etc/cron.d/vercel-cidrs
```
...
sudo nginx -t && sudo systemctl reload nginx
curl -I https://waha.elevcrm.com.br/api/health    # 401 esperado sem header
```
...
```bash
sudo tee /opt/elevcrm-waha/backup.env > /dev/null <<EOF
B2_ACCOUNT_ID=<key id>
B2_ACCOUNT_KEY=<key>
RESTIC_REPOSITORY=b2:elevcrm-waha-backup:/waha
RESTIC_PASSWORD=<senha forte armazenada no 1Password>
EOF
sudo chmod 600 /opt/elevcrm-waha/backup.env

source /opt/elevcrm-waha/backup.env
...
```cron
0 3 * * * elevcrm . /opt/elevcrm-waha/backup.env && restic backup /var/lib/docker/volumes/elevcrm-waha_waha-data --tag daily && restic forget --keep-daily 7 --keep-weekly 4 --keep-monthly 6 --prune
```
...
# em VPS de teste:
docker compose down
sudo rm -rf /var/lib/docker/volumes/elevcrm-waha_waha-data/*
. /opt/elevcrm-waha/backup.env
...
UptimeRobot (free tier, 5min) apontando pra `https://waha.elevcrm.com.br/ping` (rota pública sem auth). Alerta → email + WhatsApp via webhook do próprio app.
...
if curl -s -f http://127.0.0.1:3000/ping > /dev/null; then
  ...
  if [ "$FAILS" -ge 3 ]; then
    cd /opt/elevcrm-waha && docker compose restart waha
...
echo "*/1 * * * * elevcrm /usr/local/bin/waha-watchdog.sh" | sudo tee /etc/cron.d/waha-watchdog
```

### 8.3 Log shipping (opcional Fase 2)

Better Stack ou Datadog Agent → `docker logs elevcrm-waha`. Sem isso, logs ficam só locais e somem em rotação.
...
```
WAHA_API_BASE_URL=https://waha.elevcrm.com.br
