# Runbook — Elev CRM em VPS com CloudPanel (Nginx)
...
```bash
cd /var/www
git clone https://github.com/melgarafael/DeskcommCRM.git Elev CRM
cd /var/www/ElevCRM
...
```dotenv
REVERSE_PROXY=traefik
TRAEFIK_NETWORK=elevcrm_proxy
```

`elevcrm_proxy` não é um nome livre: é `<nome do projeto compose>_proxy`, e
o nome do projeto é o da pasta em minúsculas. Clonando em `/var/www/ElevCRM`
como acima, é `elevcrm` — logo, `elevcrm_proxy`. Clonou em outra pasta?
...
> nome de antemão evita a viagem: `elevcrm_proxy` é o nome que o próprio kit
...
segredos, escreve o `.env`, cria a rede `elevcrm_proxy`, aplica o schema,
...
```bash
docker run -d --name elevcrm-nginx-bridge --restart unless-stopped \
  --network elevcrm_proxy -p 127.0.0.1:3000:3000 \
...
`docker logs elevcrm-nginx-bridge`.
...
server {
  ...
  # `elevcrm-waha-block` (docker-compose.traefik.yml). Aqui é o mesmo bloqueio.
...
| *"Não consegui descobrir a rede Docker do seu Traefik"* | `REVERSE_PROXY=traefik` sem `TRAEFIK_NETWORK` | acrescente `TRAEFIK_NETWORK=elevcrm_proxy` |
| *"network elevcrm_proxy declared as external, but could not be found"* | a rede sumiu (`docker network prune`) | `docker network create elevcrm_proxy` |
...
| `502` no navegador | a ponte caiu | `docker start elevcrm-nginx-bridge` e veja `docker logs` |
