...
- coloca no contêiner `app` as labels de roteamento
  (`traefik.http.routers.elevcrm.rule=Host(...)`);
...
2. **PR e merge na `main`.** `publish-image.yml` dispara em push na `main` (ou
   tag `v*`) e publica **três** imagens — `elevcrm`, `elevcrm-worker` e
   `elevcrm-scheduler` — sempre na mesma versão. O build pesado roda nos
...
```bash
APP_IMAGE=elevcrm-app:local docker compose \
  -f docker-compose.prod.yml -f docker-compose.build.yml --env-file .env build app

APP_IMAGE=elevcrm-app:local APP_PULL_POLICY=never docker compose \
