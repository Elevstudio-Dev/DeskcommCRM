...
- **Banco**: Supabase local `pg17` já de pé (`supabase_db_elev-crm`), que é o alvo
...
- **Porta**: `E2E_PORT=3101`. Há um `next` vivo de **outra sessão** no worktree
  `Elev CRM-qa-main`; porta própria para não colidir, e não matei processo nenhum —
...
usar `ELEVCRM_GOV_MIGRATION_EDIT=1`. Falso positivo em gate treina a equipe a
