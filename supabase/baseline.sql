...
COMMENT ON SCHEMA "public" IS 'Elev CRM v0.1 - Migration 0001 platform_base applied 2026-04-28';
...
COMMENT ON TABLE "public"."organizations" IS 'Tenants do Elev CRM. Cada linha = 1 e-commerce cliente.';
...
-- O QUARTO CANAL (migration 0211). A coluna sobe AQUI, e não só no apêndice do
-- 0211, pela regra que o comentário acima já declara: a coluna nasce antes do
-- CHECK que a referencia. O apêndice do 0211 vem depois deste bloco no arquivo,
-- então sem esta linha o `add constraint` abaixo referenciaria coluna que ainda
-- não existe e o baseline morreria no meio — numa instalação NOVA, onde não há
-- migration nenhuma para consertar depois. `if not exists` nas duas: a segunda
-- vira no-op.
alter table public.channel_sessions
...
-- ===========================================================================
-- 0210 · grupos de WhatsApp ganham identidade (apêndice idempotente)
-- ===========================================================================
...
-- ===========================================================================
-- 0211 · Instagram entra como canal (apêndice idempotente)
-- ===========================================================================
...
-- ── 2 e 3 · OS DOIS CHECK NÃO SE REPETEM AQUI ────────────────────────────
--
-- A migration versionada faz `drop constraint` + `add constraint` com o
-- vocabulário novo — correto lá, porque uma migration roda UMA vez, em ordem.
--
-- No baseline seria um defeito, e `baseline-constraint-reconstruida.test.ts`
-- reprova: o corpo do arquivo (vindo do pg_dump) JÁ cria as duas constraints.
-- Um segundo bloco aqui embaixo as reconstruiria, e o `update.sh` de uma VPS
-- que já tenha uma linha `provider='instagram'` executaria primeiro o bloco
-- ANTIGO — que falha, porque o dado viola o vocabulário velho — deixando a
-- tabela SEM constraint entre o `drop` que passou e o `add` que não passou.
--
-- Então o vocabulário final foi escrito no bloco do CORPO, onde ele já morava.
-- Um bloco por constraint, sempre.
...
-- ===========================================================================
-- 0212 · o aviso de responsável chega sozinho (apêndice idempotente)
-- ===========================================================================
-- ANTES da varredura de propósito: ela restringe a SELECT de toda tabela
-- publicada, então rodar depois deste bloco confirma o que ele acabou de
-- fazer. O contrário deixaria uma janela em que a tabela está publicada e a
-- policy ainda alcança o anon — o defeito da 0209 de volta.
...
-- ===========================================================================
-- 0213 · a conversa ganha um setor (apêndice idempotente)
-- ===========================================================================
-- ANTES da varredura de anon da 0116 (o último bloco de funções), pela regra
-- de `varredura-anon-e-o-ultimo-bloco`: as funções nascem aqui com o
-- `revoke ... from anon` escrito, e a policy `conversations_select` nasce
-- `to authenticated` — a varredura da 0209, no fim, confirma em vez de consertar.
...
-- ===========================================================================
-- 0214 · a equipe conversa entre si (apêndice idempotente)
-- ===========================================================================
-- ANTES da varredura de anon da 0116, como a 0213: as funções nascem com o
-- `revoke ... from anon`, e a policy de SELECT de `team_messages` nasce
-- `to authenticated` ANTES de a tabela entrar na publicação.
