#!/usr/bin/env bash
# Backup do Postgres (Supabase) do Elev CRM — schema public completo
# (CRM + harness do agente). Roda no host ou num cron da VPS:
#   0 3 * * * /path/repo/scripts/backup-db.sh /var/backups/elevcrm
...
find "$DIR" -name 'elevcrm-*.dump' -mtime +"$RETENTION_DAYS" -delete
