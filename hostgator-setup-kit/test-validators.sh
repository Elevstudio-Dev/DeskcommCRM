...
* * * * * curl -fsS -H "Authorization: Bearer SEGREDO" "https://crm.elevcrm.com.br/api/v1/cron/event-log-drain" >/dev/null 2>&1
*/5 * * * * cd /root/Aula-Youtube/ElevCRM && bash hostgator-setup-kit/agent.sh >/dev/null 2>&1'
...
NOVO_TAG='# elevcrm:/root/instalacao-nova'
NOVA_URL='https://crm-novo.exemplo.com.br/api/v1/cron/event-log-drain'
cron_ok "o drain do vizinho sobrevive"  'crm.elevcrm.com.br/api/v1/cron/event-log-drain' \
        "$NOVO_TAG" "$NOVA_URL" "* * * * * curl \"$NOVA_URL\" $NOVO_TAG"
cron_ok "o agente do vizinho sobrevive" 'cd /root/Aula-Youtube/ElevCRM && bash hostgator-setup-kit/agent.sh' \
...
reexec="$(printf '%s\n' "$CRONTAB_VIZINHO" | cron_merge '# elevcrm:/root/Aula-Youtube/ElevCRM' \
          'cd /root/Aula-Youtube/ElevCRM && bash hostgator-setup-kit/agent.sh' \
          '*/5 * * * * cd /root/Aula-Youtube/ElevCRM && bash hostgator-setup-kit/agent.sh # elevcrm:/root/Aula-Youtube/ElevCRM')"
...
TAG_DRAIN="# elevcrm:${DIR}:drain"; TAG_AGENT="# elevcrm:${DIR}:agent"
...
# Caddy de OUTRO ElevCRM na mesma VPS — caía no ramo "portas livres", e a
...
dono_ok "Caddy de outro ElevCRM é encontrado" \
...
dec_ok "Caddy de OUTRO ElevCRM → bloqueia" bloqueia "80 e 443" "outro"     "crm" "caddy:2-alpine" "outro-caddy-1"
# O fixture acima nomeia "outro ElevCRM" mas usa projeto DIFERENTE ("outro" vs
...
# ElevCRM. Medido numa VPS de produção em 2026-08-24: a instalação de uma
# aula em /root/apagar7/ElevCRM viu o Caddy da produção em
# /root/ElevCRM com o MESMO projeto `elevcrm`, concluiu "é a
...
dec_ok "cópia irmã: mesmo projeto, OUTRA árvore → bloqueia" \
  bloqueia "80 e 443" "elevcrm" "elevcrm" "caddy:2-alpine" "elevcrm-caddy-1" \
  "/root/ElevCRM" "/root/apagar7/ElevCRM"
...
dec_ok "re-execução real: mesmo projeto, MESMA árvore → segue" \
  caddy "80 e 443" "elevcrm" "elevcrm" "caddy:2-alpine" "elevcrm-caddy-1" \
  "/root/ElevCRM" "/root/ElevCRM"
...
STUB
  ...
  tag_app="${img_app##*:}"
  for par in "WORKER_IMAGE:elevcrm-worker" "SCHEDULER_IMAGE:elevcrm-scheduler"; do
    chave="${par%%:*}"; repo="${par##*:}"
...
STUB
  ...
  for par in "APP_IMAGE:elevcrm" "WORKER_IMAGE:elevcrm-worker" "SCHEDULER_IMAGE:elevcrm-scheduler"; do
    chave="${par%%:*}"; repo="${par##*:}"
...
# existir depois das releases que já estão publicadas: `elevcrm-worker:1.2.1`
...
#
# A VPS deste teste é a que aconteceu de verdade: um ElevCRM no ar em
# /root/ElevCRM (Caddy publicando 80/443, projeto `elevcrm`), e o
# instalador rodando de OUTRA cópia — cuja pasta também se chama ElevCRM,
# então o projeto colide e a versão anterior dizia "é a re-execução, siga".
TMP_IRMA="$(mktemp -d)"
(
  montar_vps "$TMP_IRMA" "ElevCRM" <<'STUB'
#!/usr/bin/env bash
printf '%s\n' "$*" >> "$DOCKER_LOG"
case "$1" in
  compose) case "$*" in *" exec "*) printf 'healthy\n{"data":{"status":"healthy"}}\n' ;; esac; exit 0 ;;
  ...
  ps)      for a in "$@"; do [ "$a" = "network=host" ] && em_host=1; done
           [ "${em_host:-0}" = 1 ] && exit 0
           printf 'elevcrm-caddy-1|elevcrm|caddy:2-alpine|0.0.0.0:80->80/tcp, 0.0.0.0:443->443/tcp\n'
           exit 0 ;;
  # A árvore que pariu aquele contêiner — o dado que separa irmã de re-execução.
  inspect) case "$*" in *working_dir*) printf '/root/ElevCRM\n' ;; esac; exit 0 ;;
  network) case "$2" in inspect) exit 1 ;; esac; exit 0 ;;
esac
exit 0
STUB
  ...
  if ! printf '%s' "$saida" | grep -q 'Já existe um ElevCRM NO AR'; then
  ...
  if ! printf '%s' "$saida" | grep -q '/root/ElevCRM'; then
...
# O compose faz TrimLeft("_-") no basename. Sem isso, uma pasta /root/_elevcrm
# faz o kit calcular "_elevcrm" enquanto os contêineres carregam "elevcrm" — a
...
reexec_ok() {
  ...
  raiz="$(mktemp -d)"; dir="$raiz/elevcrm"; mkdir -p "$dir"
...
if [ ! -s "$CRONTAB_SANDBOX" ] || ! grep -q '# elevcrm:' "$CRONTAB_SANDBOX"; then
  ...
  printf '  ✓ o kit agendou %s linha(s) — todas no sandbox, o crontab real intacto\n' \
    "$(grep -c '# elevcrm:' "$CRONTAB_SANDBOX")"
fi
