#!/usr/bin/env bash
#
# ElevCRM — instalador self-host para VPS (HostGator).
#
...
decide_proxy() {  # decide_proxy <portas_ocupadas> <projeto_do_dono> <projeto_atual> <imagem> <nome> [árvore_do_dono] [árvore_atual]
  ...
  # é o basename da pasta, e toda cópia do repo se chama ElevCRM. Duas
  # árvores irmãs (/root/ElevCRM e /root/apagar7/ElevCRM) colidem no
  # nome `elevcrm` e esta linha as declarava re-execução uma da outra —
...
# Caddy — inclusive o de outro ElevCRM instalado na mesma VPS — passava
...
if [ -n "$portas_ocupadas" ]; then
  ...
  if [ -n "$_dono" ]; then
    ...
    # chamam ElevCRM e por isso compartilham o nome do projeto.
...
e, se for mesmo um Traefik, ponha REVERSE_PROXY=traefik no .env e rode de novo."
  ...
  *)
    ...
    if [ -n "$dono_arvore" ] && [ "$dono_projeto" = "$proj_atual" ] && [ "$dono_arvore" != "$_minha_arvore" ]; then
      c_red "✖ Já existe um ElevCRM NO AR nesta VPS, instalado em ${dono_arvore}."
      printf '\n%s\n'   "  Esta pasta (${_minha_arvore}) é outra cópia do repo. As duas se chamam"
      printf '%s\n'     "  ElevCRM, então o Docker dá às duas o MESMO nome de projeto"
      ...
      printf '%s\n\n'   "       COMPOSE_PROJECT_NAME=elevcrm-$(basename "${_minha_arvore}" | tr 'A-Z' 'a-z')-2"
      die "Instalação interrompida para não derrubar o ElevCRM que está no ar em ${dono_arvore}."
    fi
    ...
    printf '\n%s\n'   "  1. Já é outro ElevCRM neste servidor? Então use aquele — entre na"
...
# das imagens, e `elevcrm-worker`/`elevcrm-scheduler` só passaram a existir
# depois das releases que já estão publicadas — `elevcrm-worker:1.2.1` nunca
...
case "$_ref_final" in
  *@sha256:*)
    ...
    # `elevcrm-worker:<hash-do-app>` — uma referência que não existe em lugar
