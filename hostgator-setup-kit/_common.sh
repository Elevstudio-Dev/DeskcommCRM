...
# `/root/_elevcrm` faz o kit calcular `_elevcrm` enquanto os contêineres
# carregam `elevcrm`: a instalação deixa de se reconhecer e passa a se tratar
# como intrusa. Medido contra o docker compose v2.38.2 em `_elevcrm`,
# `-elevcrm`, `_-_crm` e `_123` — todos divergiam.
...
# diretório, e `/root/ElevCRM` e `/root/apagar6/ElevCRM` dão os dois
# `elevcrm`. Os contêineres são UM conjunto só; os `.env` são dois. Cada
...
enter_project() {
  if [ -f "$COMPOSE" ]; then :;
  elif [ -f "elevcrm/$COMPOSE" ]; then cd elevcrm;
...
# que trocar junto.
IMG_NS="ghcr.io/elevstudio-dev"
IMG_APP="${IMG_NS}/elevcrm"
IMG_WORKER="${IMG_NS}/elevcrm-worker"
IMG_SCHEDULER="${IMG_NS}/elevcrm-scheduler"
...
ghcr_status() {
  ...
  tok="$(curl -fsS --max-time 6 \
          "https://ghcr.io/token?scope=repository:${IMG_NS#*/}/${img}:pull&service=ghcr.io" 2>/dev/null \
        | sed -n 's/.*"token":"\([^"]*\)".*/\1/p')" || true
  if [ -z "$tok" ]; then printf '000'; return 0; fi
  curl -s -o /dev/null --max-time 6 -w '%{http_code}' \
    -H "Authorization: Bearer $tok" \
    -H 'Accept: application/vnd.oci.image.index.v1+json,application/vnd.docker.distribution.manifest.list.v2+json,application/vnd.docker.distribution.manifest.v2+json' \
    "https://ghcr.io/v2/${IMG_NS#*/}/${img}/manifests/${tag}" 2>/dev/null || printf '000'
}

# As TRÊS imagens existem e são públicas nesta referência?
#
# Perguntar pelas três juntas, e não só pela do app, é o ponto: `elevcrm-worker`
# e `elevcrm-scheduler` nasceram depois das releases que já existem, então
# `elevcrm-worker:1.2.1` nunca vai existir — a v1.2.1 é passado. Pinar as três
...
completar_pin_ausente() {  # completar_pin_ausente [envfile]
  ...
  for par in "WORKER_IMAGE:worker:elevcrm-worker" "SCHEDULER_IMAGE:scheduler:elevcrm-scheduler"; do
...
cron_tag() { printf '# elevcrm:%s:%s' "${PROJECT_DIR:-$PWD}" "${1:?papel da linha (drain|agent)}"; }
...
setup_update_agent_cron() {
  ...
  # a linha só funciona por acidente (instalação padrão em /root/elevcrm) e
